...
def greet_user():       # def定义一个函数，greet_user()是函数名，()中可以传递参数
    ...
    # 三引号里写这个函数是用来干嘛的，python生成函数的文档的时候，会读取三引号内的文字
    print("Hello!")
...
# 名词解释：Arguments和Parameters，定义函数时的username是parameter，调用函数时传递的michael是argument，
# username是一个变量，michael是一个值，call函数的时候，把一个值赋给一个变量
...
# 在写一个函数的时候，可以为每个参数设定一个默认值，如果某些参数有默认值，某些参数没有默认值，那些有默认值的参数要写在最后
...
def build_profile(first, last, **user_info):
    # 参数名前加**，创建一个空字典叫user_info，把接收到的所有命名参数按键值对放进这个字典
