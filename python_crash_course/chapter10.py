...
print(contents)
# open()函数返回一个对象代表这个文件pi_digits.txt，然后把这个对象赋给file_object，关键字with的作用是当不再需要访问这个文件的时候
# 关闭该文件，虽然也可以手动用close()函数关闭文件，但通常由于不知道什么时候该关闭文件而导致错误发生，用with的好处是不需要关心什么时候
# 关闭文件，python会在with代码块执行完成后自动关闭文件，read()方法读取整个文件内容
...
with open('../data/pi_digits.txt') as file_object:
    for line in file_object:
        print(line)
# 可以对file_object执行for循环，可见file_object是一个可迭代对象
# 读出来的每行之间都会有空行，因为文件每行的末尾都有换行符，并且print()本身会带一个换行符，用rstrip()去掉空行
with open('../data/pi_digits.txt') as file_object:
    for line in file_object:
        print(line.rstrip())
print()
...
# open()的第二个参数'w'告诉python用write mode打开这个文件，其他几个参数值：'r'是read mode, 'a'是append mode，'r+'是读写模式，
# 不写第二个参数默认是read mode，用'w'模式打开文件，如果这个文件不存在，open()函数会自动创建这个文件，但是如果文件已存在，则会先删除
# 文件的内容，再返回一个文件对象内容为空，write()方法不加换行符，如果想要换行，需要手动加换行符
...
# 把数据存到json文件中，json文件支持python数据结构，包括字符串列表字典等等
