# if后面的条件判断语句返回True或者False，返回True则执行if后面的代码，返回False就跳过if后面的代码
...
print(age >= 21)
print()
# 条件判断组合, and是两个条件都要True才返回True，or是两个条件只要有一个是True就返回True
age_0 = 22
age_1 = 18
...
print(f"Your admission cost is ${price}.")
print()
# if-elif-else语句适用于只对某一个条件感兴趣的情况，但有时候想检查所有感兴趣的条件，这时就要用一系列if语句，不加elif和
# else，if-elif-else语句块从上到下匹配，一旦匹配中某一个条件，后面的就跳过了，单独的if语句则每一个if都要执行
...
print()
# python代码规范：
# 写条件判断语句的时候，在比较符号两边留空格，比如：if age < 4:
