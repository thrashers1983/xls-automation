...
# 创建电动车类，继承自车类，电动车类是车类的子类，车类是电动车类的父类，子类可以继承父类的属性和方法，也可以定义子类自己的属性和方法，
# 当要创建一个子类的时候，父类必须也在当前这个文件中，并且必须出现在子类的前面，定义子类时，父类的名字要写在括号里，super()是一个
# 特殊函数，在创建子类实例的时候，super()会调用父类的__init__()方法，把其中定义的属性给子类的实例
...
# 代码规范：
# 类名首字母大写，不要加下划线，实例名和模块名用小写和下划线组合，类的方法之间空一行，类和类之间空两行，类和模块都要写docstring，
