...
yiping_son = {}
yiping_son['weight'] = 3
yiping_son['face'] = 'look like her mother'
...
#这里python有提示说变量position_increment可能未被定义，因为如果if-elif所有条件都不匹配的话，这个变量就无法被赋值，有两种方法解决这个提示：
#1.在if语句之前先定义这个变量，给它随便赋一个值 2.if语句块的最后用else匹配其余所有条件
...
#这是写字典的另一种格式，每一个键值对一行，并且每一个键值对和最后的大括号都缩进一个level，在最后的键值对的后面加一个逗号，这是好习惯
print(f"Sarah's favorite language is {favorite_languages['sarah'].title()}.")
print()
#对于字典，还可以用get()方法访问一个键的值，如果这个键存在，则正常返回该键对应的值，如果这个键不存在，则可以返回一个默认的值，这样做的目的是，
#如果访问了一个不存在的键，程序不会报错
...
#tuple是不能被更改的list，set是没有重复元素的无序的list，set和dictionary都是用大括号包裹的，容易搞混，dictionary有键值对，set都是单个元素
#
#列表嵌套字典
